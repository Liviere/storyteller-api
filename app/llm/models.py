...
class LLMModelFactory:
    ...
    def _create_model_instance(self, config: ModelConfig, **kwargs) -> Optional[BaseChatModel]:
        ...
        elif config.provider in [ModelProvider.CUSTOM_OPENAI_COMPATIBLE, ModelProvider.DEEPINFRA]:
    ...
    def _create_openai_model(self, config: ModelConfig, params: Dict[str, Any]) -> Optional[ChatOpenAI]:
    ...
    def _create_openai_compatible_model(self, config: ModelConfig, params: Dict[str, Any]) -> Optional[ChatOpenAI]:
        """Create OpenAI-compatible model instance (LM Studio, etc.)"""
        try:
            return ChatOpenAI(
                model_name=config.name,
                openai_api_base=config.base_url,
                openai_api_key=config.api_key or "not-needed",  # Some endpoints don't require API key
                **params
            )
        except Exception as e:
