...
class StoryAnalysisChain(BaseStoryChain):
    ...
    def _build_chains(self):
        ...
        self._sentiment_chain = (
            AVAILABLE_PROMPTS["sentiment_analysis"] | model | StrOutputParser()
        )

        # Genre classification chain
        self._genre_chain = (
            AVAILABLE_PROMPTS["genre_classification"] | model | StrOutputParser()
        )

        # Full story analysis chain
        self._analysis_chain = (
            AVAILABLE_PROMPTS["story_analysis"] | model | StrOutputParser()
        )
...
class StorySummaryChain(BaseStoryChain):
    ...
    async def summarize_story(
        ...
        return await self.arun(
            content=content, summary_length=summary_length, focus=focus
        )
...
class StoryImprovementChain(BaseStoryChain):
    ...
    def _build_chains(self):
        ...
        self._improvement_chain = (
            AVAILABLE_PROMPTS["story_improvement"] | model | StrOutputParser()
        )

        # Grammar correction chain
        self._grammar_chain = (
            AVAILABLE_PROMPTS["grammar_correction"] | model | StrOutputParser()
        )

        # Style transformation chain
        self._style_chain = (
            AVAILABLE_PROMPTS["style_transformation"] | model | StrOutputParser()
        )
...
class StoryCreativeChain(BaseStoryChain):
    ...
    def _build_chains(self):
        ...
        self._ending_chain = (
            AVAILABLE_PROMPTS["story_alternative_ending"] | model | StrOutputParser()
        )

        # Character dialogue chain
        self._dialogue_chain = (
            AVAILABLE_PROMPTS["character_dialogue"] | model | StrOutputParser()
        )

        # Story continuation chain
        self._continuation_chain = (
            AVAILABLE_PROMPTS["story_continuation"] | model | StrOutputParser()
        )
...
def create_story_summary_chain(
    model_name: Optional[str] = None, **model_params
) -> StorySummaryChain:
