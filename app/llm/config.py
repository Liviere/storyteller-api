...
class LLMConfig:
    ...
    def _load_config(self):
        ...
        try:
            ...
            print(f"Warning: LLM config file not found at {config_path}. Using fallback configuration.")
        ...
        for model_name, model_data in models_config.items():
            ...
            if not base_url:
                ...
                if base_url_env:
                    base_url = os.getenv(base_url_env, provider_config.get('default_base_url'))
                else:
        ...
        for task_name, task_data in tasks_config.items():
            ...
            env_var = yaml_config.get('settings', {}).get('env_overrides', {}).get(task_name)
            if env_var:
    ...
    def _load_fallback_config(self):
        ...
        self.enable_monitoring = os.getenv("LLM_ENABLE_MONITORING", "true").lower() == "true"
    ...
    def is_model_available(self, model_name: str) -> bool:
        ...
        if config.provider in [ModelProvider.OPENAI, ModelProvider.DEEPINFRA]:
            return (config.api_key is not None and 
                   ...
                   not config.api_key.startswith("your-"))
    ...
    def get_model_debug_info(self, model_name: str) -> Dict[str, Any]:
        ...
        if config.api_key:
            debug_info["api_key_is_placeholder"] = (
                config.api_key.startswith("your-") or 
                config.api_key == "your-openai-api-key-here"
            )
            debug_info["api_key_preview"] = config.api_key[:10] + "..." if len(config.api_key) > 10 else config.api_key
