...
class TestLLMServiceInitialization:
    ...
    def test_service_has_required_methods(self):
        ...
        required_methods = [
            'generate_story', 'analyze_story', 'summarize_story',
            'improve_story', 'get_available_models', 'get_usage_stats'
        ]
...
class TestLLMServiceMethods:
    ...
    async def test_generate_story_basic(self, service):
        ...
        with patch('app.llm.services.create_story_generation_chain') as mock_chain_creator, \
    ...
    async def test_analyze_story_basic(self, service, sample_story_content):
        ...
        with patch('app.llm.services.create_story_analysis_chain') as mock_chain_creator, \
             patch.object(service, '_log_request'), \
             patch.object(service, '_update_usage_stats'):
            
            mock_chain = AsyncMock()
            # Mock analyze_story method to return just the analysis string
            mock_chain.analyze_story.return_value = "This is a fantasy story with positive sentiment."
            mock_chain_creator.return_value = mock_chain
            ...
            assert "metadata" in result
            assert result["analysis"] == "This is a fantasy story with positive sentiment."
    ...
    async def test_summarize_story_basic(self, service, sample_story_content):
        ...
        with patch('app.llm.services.create_story_summary_chain') as mock_chain_creator, \
             patch.object(service, '_log_request'), \
             patch.object(service, '_update_usage_stats'):
            
            mock_chain = AsyncMock()
            # Mock summarize_story method to return just the summary string
            mock_chain.summarize_story.return_value = "A brief summary of the adventure."
            mock_chain_creator.return_value = mock_chain
    ...
    async def test_improve_story_basic(self, service, sample_story_content):
        ...
        with patch('app.llm.services.create_story_improvement_chain') as mock_chain_creator, \
             patch.object(service, '_log_request'), \
             patch.object(service, '_update_usage_stats'):
    ...
    def test_get_available_models(self, service):
        ...
        with patch.object(service.model_factory, 'get_available_models') as mock_get_models:
...
class TestLLMServiceErrorHandling:
    ...
    async def test_service_timeout_handling(self, service):
        ...
        with patch('app.llm.services.create_story_generation_chain') as mock_chain_creator:
            mock_chain = AsyncMock()
            mock_chain.generate_story.side_effect = asyncio.TimeoutError("Request timed out")
            mock_chain_creator.return_value = mock_chain
...
class TestLLMServiceParameterValidation:
    ...
    @pytest.mark.asyncio
    @pytest.mark.llm_mock
    async def test_analyze_story_parameter_validation(self, service, sample_story_content):
    ...
    async def test_model_fallback_on_primary_failure(self, service):
        ...
        with patch('app.llm.services.create_story_generation_chain') as mock_chain_creator:
            ...
            mock_chain_creator.side_effect = [
                ...
                AsyncMock(ainvoke=AsyncMock(return_value={
