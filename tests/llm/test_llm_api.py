...
class TestLLMGenerateEndpoint:
    ...
    @pytest.mark.llm_mock
    def test_generate_story_valid_request(
    ...
    @pytest.mark.llm_mock
    def test_generate_story_all_samples(
        self, client: TestClient, mock_llm_service, sample_generation_requests
    ):
        ...
        try:
            for i, request in enumerate(sample_generation_requests):
                ...
                # Verify that mock response is used
                assert (
                    data["story"] == "Once upon a time, in a land far away..."
                ), f"Request {i} not using mock story"
                assert (
                    data["metadata"]["model"] == "test-model"
                ), f"Request {i} not using mock model"

            # Verify mock was called for each request
            assert mock_llm_service.generate_story.call_count == len(
...
class TestLLMAnalyzeEndpoint:
    ...
    @pytest.mark.llm_mock
    def test_analyze_story_valid_request(
        self, client: TestClient, mock_llm_service, sample_analysis_requests
    ):
        ...
        try:
            ...
            # Verify that mock response is used
            assert (
                data["analysis"] == "This is a fantasy story with positive sentiment."
            )
            assert data["metadata"]["model"] == "test-model"
    ...
    def test_analyze_story_invalid_analysis_type(
        self, client: TestClient, mock_llm_service, sample_story_content
    ):
...
class TestLLMSummarizeEndpoint:
    ...
    @pytest.mark.llm_mock
    def test_summarize_story_valid_request(
        self, client: TestClient, mock_llm_service, sample_summary_requests
    ):
...
class TestLLMImproveEndpoint:
    ...
    @pytest.mark.llm_mock
    def test_improve_story_valid_request(
        self, client: TestClient, mock_llm_service, sample_improvement_requests
    ):
